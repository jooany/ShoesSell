...
import test.resell.dto.ResellCommentDto;
import test.util.DbcpBean;

public class ResellCommentDao {
	private static ResellCommentDao dao;
	
	static {
		dao=new ResellCommentDao();
	}
	private ResellCommentDao() {}
	public static ResellCommentDao getInstance() {
		return dao;
	}
	...
	//댓글 내용을 수정하는 메소드
	public boolean update(ResellCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			...
			String sql = "UPDATE resell_comment"
					+ " SET content=?"
					+ " WHERE num=?";
			pstmt = conn.prepareStatement(sql);
	...
	//댓글 목록을 리턴하는 메소드
	public List<ResellCommentDto> getList(ResellCommentDto dto2){
		List<ResellCommentDto> list=new ArrayList<ResellCommentDto>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			String sql = "SELECT *" + 
					...
					"		comment_group, deleted, resell_comment.regdate, profile" + 
					"		FROM resell_comment" + 
					"		INNER JOIN users" + 
					"		ON resell_comment.writer = users.id" + 
					"		WHERE ref_group=?" + 
	...
	public int getSequence() {
		...
		try {
			conn = new DbcpBean().getConn();
			//select 문 작성
			String sql = "SELECT resell_comment_seq.NEXTVAL AS seq"
					+ " FROM DUAL";
			pstmt = conn.prepareStatement(sql);
			// ? 에 바인딩 할게 있으면 여기서 바인딩한다.
	...
	//댓글 추가
	public boolean insert(ResellCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			...
			String sql = "INSERT INTO resell_comment"
