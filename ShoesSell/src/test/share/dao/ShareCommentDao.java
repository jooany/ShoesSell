...
import test.share.dto.ShareCommentDto;
import test.util.DbcpBean;

public class ShareCommentDao {
	private static ShareCommentDao dao;
	
	static {
		dao=new ShareCommentDao();
	}
	...
	//댓글 내용을 수정하는 메소드
	public boolean update(ShareCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			...
			String sql = "UPDATE share_comment"
					+ " SET content=?"
					+ " WHERE num=?";
			pstmt = conn.prepareStatement(sql);
	...
	//댓글 목록을 리턴하는 메소드
	public List<ShareCommentDto> getList(ShareCommentDto dto2){
		List<ShareCommentDto> list=new ArrayList<ShareCommentDto>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			String sql = "SELECT *" + 
					...
					"		comment_group, deleted, share_comment.regdate, profile" + 
					"		FROM share_comment" + 
					"		INNER JOIN users" + 
					"		ON share_comment.writer = users.id" + 
					"		WHERE ref_group=?" + 
					"		ORDER BY comment_group DESC, num ASC) r1)" + 
	...
	public int getSequence() {
		...
		try {
			conn = new DbcpBean().getConn();
			//select 문 작성
			String sql = "SELECT share_comment_seq.NEXTVAL AS seq"
					+ " FROM DUAL";
			pstmt = conn.prepareStatement(sql);
			// ? 에 바인딩 할게 있으면 여기서 바인딩한다.
	...
	//댓글 추가
	public boolean insert(ShareCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			...
			String sql = "INSERT INTO share_comment"
